...
class ANN():
    ...
    def __init__(self, base_population_path, label_path, experimental_size,
    ...
    def supervised_evaluation(self, top_x=2):
        ...
        for embed_vec in self.base_population:
            predictions.append(self.assign_label(embed_vec))
        self.top_x = np.sum([label in pred for label, pred in zip(self.labels, predictions)])/len(self.labels)
        self.assigned_classes = np.sum([label == pred[0] for label, pred in zip(self.labels, predictions)]) /len(self.labels)
